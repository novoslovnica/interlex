...
- ⬜ **Grammar engine produces Proto-Slavic, not modern Interslavic, endings** across all noun/adjective/pronoun/numeral/verb paradigms. The `ending_allophones` DB table that `lib/grammar/endingLoader.ts` treats as an "override" is seeded (`scripts/db/seed-endings.ts`) with the *same* Proto-Slavic values, so it corrects nothing. `lib/corpus/tokenizer/dbAnalyzer.ts`'s `matchByStemPrefix` fallback silently masks this for corpus tagging (feats come back empty). Full detail in AGENTS.md's "Corpus Tokenizer: DbAnalyzer Architecture" section.
...
- ✅ **FTS5 table unused as FTS** (fixed 2026-07-22) — `lexemes_text`/`lexeme_allophones_text` were queried with `LIKE '%term%'`, which can't use an FTS5 index. Both virtual tables were rebuilt with `tokenize='trigram'` (verified byte-for-byte identical result sets vs. the old `LIKE` scan across Latin/Cyrillic/short-query samples) and `getDictItems` now uses `MATCH` for search terms of 3+ characters, falling back to parameterized `LIKE` below that (trigram tokenization can't index patterns shorter than 3 chars).
