...
/api/synonyms/second-level        -- POST, public again since 2026-08-12
...
- ✅ **N+1 queries on `/words/[id]`** (fixed 2026-08-12, roadmap #10) — the per-language queries went away with the consolidated `translations` table; `getItem()` is wrapped in `React.cache`.
- ⬜ **Widespread `any` usage** despite the project's "avoid `any`" rule (~130 `no-explicit-any` lint errors as of 2026-09-15). Can no longer grow: CI fails on any new lint/tsc error relative to `scripts/ci/baselines/` (see `scripts/ci/check-baseline.mjs`).
