...
interface SynonymsClientProps {
    ...
    onUpdateSynonyms: (sourceMeaningId: number, targetMeaningIds: number[]) => Promise<void>
}
...
export function SynonymsClient({ initialWords, onUpdateSynonyms }: SynonymsClientProps) {
    ...
    const [synonymSearchResults, setSynonymSearchResults] = useState<SimpleMeaning[]>([])
    ...
    const [attachedSynonyms, setAttachedSynonyms] = useState<SimpleMeaning[]>([])
    ...
    useEffect(() => {
        ...
        const delayDebounce = setTimeout(async () => {
            ...
            try {
                const res = await fetch(`/api/meanings/search?query=${encodeURIComponent(synonymSearchQuery)}`)
                if (res.ok) {
                    const data = await res.json()
                    setSynonymSearchResults(data.filter((m: SimpleMeaning) => m.id !== selectedMeaningId))
                }
            } catch (e) {
                console.error(e)
            } finally {
        ...
        return () => clearTimeout(delayDebounce)
    }, [synonymSearchQuery, selectedMeaningId])
    ...
    useEffect(() => {
        if (selectedMeaningId && activeWord) {
            ...
            if (meaning) {
                const existing: SimpleMeaning[] = meaning.synonymsSource
                    .map((s) => s.target)
    ...
    const handleToggleSynonym = (meaning: SimpleMeaning) => {
        const isAttached = attachedSynonyms.some((s) => s.id === meaning.id)
        if (isAttached) {
            setAttachedSynonyms(attachedSynonyms.filter((s) => s.id !== meaning.id))
        } else {
            setAttachedSynonyms([...attachedSynonyms, meaning])
        }
    }
    ...
    const handleSave = () => {
        ...
        startTransition(async () => {
            try {
                ...
                await onUpdateSynonyms(selectedMeaningId, ids)
                ...
                setWords(prev => prev.map(w => {
                    ...
                    return {
                        ...
                        meanings: w.meanings.map(m => {
                            ...
                            return {
                                ...m,
                                synonymsSource: attachedSynonyms.map(s => ({
                                    id: 0,
                                    proximity: 1,
            ...
            <div className="lg:col-span-4 bg-transparent h-full overflow-y-auto space-y-3 pr-2 flex flex-col min-h-0">
                ...
                <div className="flex-1 overflow-y-auto space-y-1">
                    ...
                    {words.map((w) => {
                        ...
                        const totalSyn = w.meanings.reduce((sum, m) => sum + m.synonymsSource.length, 0)
                        return (
                            <div
                                key={w.id}
                    ...
                    <div className="space-y-5 flex-1 flex flex-col">
                        ...
                        {activeMeaning ? (
                                    ...
                                    <div className="p-3 border border-dashed rounded-lg bg-muted/10 flex flex-wrap gap-1.5 min-h-[50px] max-h-[120px] overflow-y-auto">
                                        {attachedSynonyms.length > 0 ? (
                                            ...
                                            <span className="text-xs text-muted-foreground italic p-1">
                                                Список синонимов пуст. Используйте поиск ниже, чтобы добавить значения с нуля.
                                            </span>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="flex items-center justify-center h-full text-sm text-muted-foreground py-12">
                                Выберите значение слова для управления синонимами
                            </div>
                        )}
                    </div>
                ) : (
