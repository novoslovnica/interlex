...
export function readme(meta: { version: string; builtAt: string; commit: string | null; stats: Record<string, { words: number; entries: number; classes: number }> }): string {
    const lines = [
        ...
        "",
        "isv_Latn - Latin script: the standard spelling (člověk, jezyk, sut, prijatelj) and the common",
        "           simplified one (človek); suggestions lead to the standard.",
        "isv_Cyrl - Cyrillic script (чловєк/чловек, језык, сут, пријатељ).",
        "",
