/**
 * Краулер старого сайта izvesti.info, перенесённого (с поломанным SEF-
 * роутингом) на interslavic.news/izvesti.info/. Обходит роутинг через прямой
 * Joomla-компонент index.php?option=com_content&view=article&id=N. См.
 * scripts/crawl-isv-wikipedia.ts за общим паттерном идемпотентности —
 * externalId "izvesti:<id>" + псевдо-revision (хэш текста).
 *
 * Запуск: npm run crawl:izvesti-info
 * Ограничить (для теста): CRAWL_LIMIT=10 npm run crawl:izvesti-info
 */
...
import { listIzvestiArticles, MAX_ARTICLE_ID_SCAN } from "@/lib/corpus/sources/izvestiClient"
...
async function main() {
    ...
    for await (const article of listIzvestiArticles()) {
        if (CRAWL_LIMIT !== undefined && seen >= CRAWL_LIMIT) break
        seen++

        const externalId = `izvesti:${article.id}`
