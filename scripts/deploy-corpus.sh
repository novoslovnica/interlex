...
# Именно --baseline, а не обычный прогон: перенесённый корпус уже разобран
# под этот словарь, пересчитывать нечего. А без флага цикл увидел бы весь
# словарь изменившимся — шагом выше compute:frequency сдвинул updatedAt у
# всех 24 440 лексем — и ушёл бы в многочасовой пересчёт на живом сервере.
info "Только точка отсчёта: корпус уже соответствует словарю."
run_remote "точка отсчёта цикла" "npm run corpus:refresh -- --baseline"
